...
writer = SummaryWriter(log_dir = "/home/users/qianhao/PycharmProjects/798Polyp/new_aspp_final/train_SHAT/")




# os.environ['CUDA_VISIBLE_DEVICES'] = '4, 2, 5, 7, 0, 1, 3'
# device_ids = [4, 2, 5, 7, 0, 1, 3]

# os.environ['CUDA_VISIBLE_DEVICES'] = '4, 5'
device_ids = [0,1,2,3,4,5,6,7]
# device = torch.evice("cuda:{}".format(4))
...
#             gts3 = F.interpolate(gts, size=(P4.shape[3], P4.shape[2]), mode='bilinear', align_corners=False)
            # ---- loss function ----

            loss_P1 = structure_loss(P1, gts)
            loss_P2 = structure_loss(P2, gts1)
            loss_P3 = structure_loss(P3, gts2)
#             loss_P4 = structure_loss(P4, gts3)

            # loss_P2 = structure_loss(P2, gts)
            # loss = loss_P1 + loss_P2
            loss = (loss_P1 + loss_P2 + loss_P3)/3.0
            # loss = 0.5*loss_P1 + 0.3*loss_P2 + 0.2*loss_P3
            # ---- backward ----
            loss.backward()
    ...
    if epoch % 1 == 0:
        ...
        for j in range(0, 5):
            ...
            #             loggers_and_handlers[j][0].info('epoch: {}, dataset: {}, dice: {}, iou: {}'.format(epoch, dataset, dataset_dice,dataset_iou))
            ...
            #             torch.save(model.state_dict(), save_path + 'train_model_SHAT_'+str(epoch)+'_' + dataset + '.pth')
            if dataset_dice > best[j] and dataset_iou > best1[j]:
                ...
                torch.save(model.state_dict(),
                           save_path + 'train_model_SHAT_doublebest'+dataset+'.pth')
                ...
                loggers_and_handlers[j][0].info(
                    '{}###########################################################################dice:{} bestiou:{} '.format(
                        epoch, dataset_dice, dataset_iou))
                torch.save(model.state_dict(),
                           save_path + 'train_model_SHAT_bestiou'+dataset+'.pth')
                ...
                loggers_and_handlers[j][0].info(
                    '{}############################################################################bestdice:{} iou:{} '.format(
                        epoch, dataset_dice, dataset_iou))
                torch.save(model.state_dict(),
                           save_path + 'train_model_SHAT_bestdice'+dataset+'.pth')
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str,
                        default='/home/users/qianhao/model_newd/model_pth_model_SHAT/')

    opt = parser.parse_args()
    # logging.basicConfig(filename='/home/users/qianhao/log_newd/train_log_train_model_SHAT.log',
    #                     format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
    #                     level=logging.INFO, filemode='a', datefmt='%Y-%m-%d %I:%M:%S %p')

    # ---- build models ----
    model = SHAT().to(device_ids[0])
    ...
    for i in range(5):
        ...
        # 创建文件处理器，将日志写入到对应的文件
        file_handler = logging.FileHandler(f'/home/users/qianhao/log_newd/SHAT_log/train_SHAT_{d[i]}.log', mode='a')
...
#     plt.show()
#     plt.savefig('autodl-tmp/train_model_SHAT')
